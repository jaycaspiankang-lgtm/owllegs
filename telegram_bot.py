...
# Database
DATABASE = "/data/telegram-bets.db" if os.path.isdir("/data") else "telegram-bets.db"
        ...
        if live_data:
            for game in live_data:
                ...
                if (
                    ...
                    or any(word in pick_lower for word in home.split())
                    or any(word in pick_lower for word in away.split())
                ):
        ...
        try:
            ...
            for event in data.get("events", []):
                ...
                if len(competitors) >= 2:
                    ...
                    game["score"] = (
                        f"{game['away_abbrev']} {game['away_score']} - {game['home_abbrev']} {game['home_score']}"
                    )
...
async def parlays_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not parlays:
        await update.message.reply_text(
            "You have no open parlays! Create one with /parlay"
        )
        return
...
async def parlay_lost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Parlay #{parlay_id} marked as LOST. Better luck next time!"
    )
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Reading your betting slip... (this may take a moment)"
    )
    ...
    try:
        ...
        parlay_id = add_parlay(
            user.id, user.first_name, chat_id, legs, stake=stake, source="screenshot"
        )
        parlay = get_parlay(parlay_id)
